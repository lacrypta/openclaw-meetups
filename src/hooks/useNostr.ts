...
export function useNostr() {
  ...
  // On mount, verify saved NIP-07 session is still valid
  useEffect(() => {
    if (saved.pubkey && saved.method === 'nip07') {
      checkNip07().then(async (hasExt) => {
        if (hasExt) {
          try {
            const currentPubkey = await loginWithNip07();
            if (currentPubkey !== saved.pubkey) {
              // Extension has different key now
              saveState(currentPubkey, 'nip07');
              setState(s => ({ ...s, pubkey: currentPubkey }));
            }
          } catch {
            // Extension denied or failed - clear saved state
            saveState(null, null);
